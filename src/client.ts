...
import { buildFetchHandler, type FetchHandler, type FetchHandlerObject } from '@atcute/client';
import { mergeHeaders } from '@atcute/client/utils/http';

import type { At, Procedures, Queries } from '@atcute/client/lexicons';
...
export type QueryRequestOptions<TDef> =
	& BaseRequestOptions
	& (TDef extends { response: infer TResponse } ? TResponse extends { json: any }
...
export type ProcedureRequestOptions<TDef> =
	& BaseRequestOptions
	& (TDef extends { response: infer TResponse } ? TResponse extends { json: any }
...
export type SuccessClientResponse<TDef, TInit> = BaseClientResponse & {
	...
	data: TInit extends { as: infer TFormat }
		? TFormat extends ResponseFormat ? FormattedResponse<TDef>[TFormat]
		: TFormat extends null ? null
		: never
		: TDef extends { response: { json: infer TData } } ? TData
		: never;
};
